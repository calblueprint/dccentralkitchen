...
// 48px: Size of EBTBar
// 15px: Size of Title (Kind of arbitrary for now?)
export const truncateStoreName = function sync(storeName, screenWidth) {
  const acceptableLimit = (screenWidth - 130 - 48) / 15;
  if (storeName.length > acceptableLimit) {
    return storeName.slice(0, acceptableLimit) + '...';
  }
  return storeName;
};
