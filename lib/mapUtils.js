        ...
        Alert.alert(
          ...
          [
            { text: 'Not now', style: 'cancel', onPress: async () => null },
            {
              ...
              style: 'default',
              onPress: async () =>
                navigation.navigate('Settings', { screen: 'Notifications' }), // TODO: link to specific page in settings
            },
          ]
        );
      }
...
export async function orderStoresByDistance(region, inputStores) {
  const sortedStores = [];
  // Display distance in the StoreList
  inputStores.forEach(async (store) => {
    const currStore = store;
    currStore.distance = findStoreDistance(region, store);
...
export function useCurrentLocation(setInitialLocation, setLocationPermissions) {
  useEffect(() => {
    ...
    getCurrentLocation();
  }, [setInitialLocation, setLocationPermissions]);
}

/**
 * Sets initial stores in state.
 */
export function useInitialStores(setStores) {
  useEffect(() => {
    const populateInitialStores = async () => {
      try {
        ...
        console.error('[MapScreen] (useInitialStores) Airtable:', err);
        logErrorToSentry({
          screen: 'MapScreen',
          function: 'useInitialStores',
          error: err,
        });
      }
    };
...
export function filterStore(searchStr) {
  return (store) => {
    return (
...
// If location permissions are granted and location is found, order stores by distance.
export function useSortedStores(
  ...
  useEffect(() => {
    ...
    const orderLoadedStores = async () => {
      const sortedStores = await orderStoresByDistance(initialLocation, stores);
      setStores(sortedStores);
      setSorted(true);
    };
    orderLoadedStores();
  }, [initialLocation, stores, locationPermissions]);
}
