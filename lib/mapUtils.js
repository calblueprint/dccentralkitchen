...
import { googlePlaceId, initialRegion } from '../constants/Map';
...
export function useCurrentLocation(setInitialLocation, setLocationErrorMsg) {
  useEffect(() => {
    const getCurrentLocation = async () => {
      try {
        ...
        setInitialLocation(initialRegion);
        setLocationErrorMsg(err.message);
      }
    };
    getCurrentLocation();
  }, [setInitialLocation, setLocationErrorMsg]);
}

/**
 * Sets initial stores in state.
 */
export function useInitialStores(initialLocation, setStores) {
  useEffect(() => {
    if (!initialLocation) {
      return;
    }
