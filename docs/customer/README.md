...
3. Add environment files
   ...
   - We use `.env.generator` as the config file for the `airtable-schema-generator` package. See the [Airtable section](#working-with-airtable) for more. Duplicate `.env.generator.example`, name it `.env.generator`, and fill in relevant variables.
...
These code quality and lifecycle improvements were originally added to this repo in [PR #23](https://github.com/calblueprint/dccentralkitchen/pull/23).
...
::: tip
See [PR #48](https://github.com/calblueprint/dccentralkitchen/pull/48) for when we first refactored the existing code to use the wrapper API.
:::
